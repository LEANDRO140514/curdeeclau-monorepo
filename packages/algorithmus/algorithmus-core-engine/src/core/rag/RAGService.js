...
function resolveRagDeps(arg1, arg2) {
    if (arg2 !== undefined) {
        if (!isLoggerLike(arg1)) {
            throw new Error("RAGService: el segundo argumento solo es válido si el primero es logger");
        }
...
class RAGService {
    ...
    async query(input) {
        ...
        try {
            ...
            let documents = [];
            try {
                documents = await this.searchVectorStore(input.tenantId.trim(), normalizedQuery, usedTopK);
            }
            catch (searchErr) {
                if (m) {
            ...
            if (documents.length === 0) {
                ...
                log.info({ step: "rag_query_empty", usedTopK }, "rag query empty");
            }
            else {
                if (m) {
    ...
    validate(input) {
        ...
        if (input.topK !== undefined) {
            if (!Number.isInteger(input.topK) ||
                ...
                throw new Error(`RAGService: topK debe ser un entero entre ${MIN_TOP_K} y ${MAX_TOP_K}`);
            }
        }
    }
    async searchVectorStore(tenantId, query, topK) {
