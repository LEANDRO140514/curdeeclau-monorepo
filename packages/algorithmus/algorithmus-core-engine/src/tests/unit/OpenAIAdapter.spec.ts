...
//
// Validates the OpenAIAdapter against the canonical
...
import { OpenAIAdapter, type HttpFetch } from '../../infra/providers/openai/OpenAIAdapter';
...
describe('OpenAIAdapter', () => {
  ...
  it('generate() envia model del request si se proporciona', async () => {
    responses.push(mockChatResponse('ok', { model: 'gpt-4-turbo' }));
    adapter = createAdapter();
  ...
  it('generate() usa defaultModel si request no trae model', async () => {
    responses.push(mockChatResponse('ok', { model: 'gpt-4o' }));
    adapter = createAdapter();
