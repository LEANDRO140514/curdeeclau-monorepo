...
//
// Validates the DeepSeekAdapter against the canonical
...
import { DeepSeekAdapter, type HttpFetch } from '../../infra/providers/deepseek/DeepSeekAdapter';
...
describe('DeepSeekAdapter', () => {
  ...
  it('generate() devuelve LLMResponse con texto mapeado', async () => {
    responses.push(mockChatResponse('Hola, soy DeepSeek'));
    adapter = createAdapter();
    ...
    expect(result.text).toBe('Hola, soy DeepSeek');
    expect(result.provider).toBe('deepseek');
    expect(result.model).toBe('deepseek-chat');
  });

  it('generate() envia model del request si se proporciona', async () => {
    responses.push(mockChatResponse('ok', { model: 'deepseek-reasoner' }));
    adapter = createAdapter();
  ...
  it('generate() usa defaultModel si request no trae model', async () => {
    responses.push(mockChatResponse('ok', { model: 'deepseek-chat' }));
    adapter = createAdapter();
  ...
  it('providerMetadata incluye prefijo deepseek_', async () => {
    responses.push(mockChatResponse('ok'));
    adapter = createAdapter();
    ...
    expect(result.providerMetadata).toHaveProperty('deepseek_id');
    expect(result.providerMetadata).toHaveProperty('deepseek_created');
  });
});
